    ...
    <section id="planos" className="py-16 md:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-5 md:px-8">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {plans.map((plan) => (
            <article key={plan.name} className={selectedPlan === plan.id ? 'rounded-[22px] border-2 border-brand-yellow bg-white p-6 shadow-hero transition duration-300 hover:-translate-y-1 dark:bg-night-card' : 'rounded-[22px] border border-brand-blue/15 bg-white p-6 shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-hero dark:border-white/10 dark:bg-night-card'}>
              <div className="flex items-center justify-between gap-3">
                <h3 className="font-display text-2xl font-semibold text-ink dark:text-white">{plan.name}</h3>
                <CheckCircle2 size={20} className={selectedPlan === plan.id ? 'text-brand-yellow' : 'text-brand-blue dark:text-brand-sky'} />
              </div>
              <p className="mt-3 text-sm font-extrabold uppercase tracking-[0.12em] text-brand-blue dark:text-brand-sky">{plan.range}</p>
              <div className="mt-6">
                <span className="font-display text-4xl font-semibold text-ink dark:text-white">{plan.price}</span>
                <span className="ml-1 text-sm font-bold text-slate-500 dark:text-slate-400">{plan.note}</span>
              </div>
              <p className="mt-5 text-sm leading-7 text-slate-600 dark:text-slate-300">Disponibilidade conferida antes do cadastro e do pagamento. Cancelamentos ou remarcações com no mínimo 48h de antecedência.</p>
