    ...
    <section className="mx-auto max-w-6xl px-4 py-14 sm:px-5 md:px-8 md:py-20">
      ...
      <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
        {benefits.map((benefit) => {
          ...
          return (
            <article key={benefit.title} className="flex items-start gap-4 rounded-[22px] border border-brand-blue/15 bg-white p-5 text-left shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-hero dark:border-white/10 dark:bg-night-card md:block md:p-6 md:text-center">
              <div className="grid h-12 w-12 shrink-0 place-items-center rounded-full border border-brand-blue/15 bg-brand-soft text-brand-blue shadow-sm dark:border-white/10 dark:bg-night-soft dark:text-brand-sky md:mx-auto md:h-16 md:w-16">
                <Icon size={24} />
              </div>
              <div>
                <h3 className="font-display text-xl font-semibold text-ink dark:text-white md:mt-5">{benefit.title}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300 md:mt-3 md:leading-7">{benefit.description}</p>
              </div>
            </article>
          );
        })}
