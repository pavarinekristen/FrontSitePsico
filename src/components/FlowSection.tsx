    ...
    <section id="fluxo" className="mx-auto max-w-6xl px-4 py-16 sm:px-5 md:px-8 md:py-24">
      ...
      <div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {steps.map((step, index) => {
          ...
          return (
            <article key={step.title} className={index === steps.length - 1 ? 'flex items-start gap-4 rounded-[22px] border border-brand-yellow/80 bg-brand-navy p-5 text-white shadow-hero md:block md:p-6 lg:col-span-2' : 'flex items-start gap-4 rounded-[22px] border border-brand-blue/15 bg-white p-5 shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-hero dark:border-white/10 dark:bg-night-card md:block md:p-6'}>
              <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-brand-yellow text-ink md:h-12 md:w-12">
                <Icon size={22} />
              </div>
              <div>
                <div className={index === steps.length - 1 ? 'text-xs font-extrabold uppercase tracking-[0.14em] text-brand-yellow md:mt-5' : 'text-xs font-extrabold uppercase tracking-[0.14em] text-brand-blue dark:text-brand-sky md:mt-5'}>Passo {index + 1}</div>
                <h3 className={index === steps.length - 1 ? 'mt-1 font-display text-xl font-semibold md:mt-2 md:text-2xl' : 'mt-1 font-display text-xl font-semibold text-ink dark:text-white md:mt-2 md:text-2xl'}>{step.title}</h3>
                <p className={index === steps.length - 1 ? 'mt-2 text-sm leading-6 text-white/80 md:mt-3 md:leading-7' : 'mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300 md:mt-3 md:leading-7'}>{step.description}</p>
              </div>
            </article>
          );
        })}
