...
export function Header() {
  ...
  return (
    <header className="sticky top-3 z-50 mx-auto mt-3 w-[calc(100%-1.5rem)] max-w-6xl rounded-[18px] border border-brand-blue/15 bg-white/75 px-4 py-3 shadow-nav backdrop-blur sm:w-[calc(100%-2rem)] md:px-5">
        ...
        <nav className="mt-3 flex flex-col gap-1 border-t border-brand-blue/10 pt-3 md:hidden">
          {navItems.map((item) => (
            <a
              ...
              className="rounded-lg px-3 py-3 text-sm font-bold text-slate-600 transition hover:bg-brand-soft hover:text-brand-blue"
            >
              {item.label}
            </a>
          ))}
